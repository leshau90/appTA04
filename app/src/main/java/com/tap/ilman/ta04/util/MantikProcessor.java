...
public class MantikProcessor {
	...
	static void addAppropriateTerm(PExpr outer, PExpr innerExpr,
		...
		// dengan +, agar child elemet ditambahkan seperti biasa
	...
	static PExpr createExpr(Queue<Lexeme> lexTable, Param p ) {

		PExpr root = new PExpr(p);
		PExpr mid = root;
		PExpr inner = root;

		ArrayList<ReferenceItem> DepthTracker = new ArrayList<ReferenceItem>();
		// first feed all positive
		Object[] feed = { null, TokenM.ADD, lexTable.remove() };
		Lexeme lexeme;
